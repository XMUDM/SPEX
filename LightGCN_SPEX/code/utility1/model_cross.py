...
class LightGCN(BasicModel):
    ...
    def compute_scores(self, hidden, mask):
        ...
        b = self.u2e_t_new[:-1]  # n_nodes x latent_size
    ...
    def forward(self, users, items, labels, slice_indices, trust_data, flag=0):
        ...
        self.u2e_r_new = nn.Parameter(u2e_new[0].view(self.num_users+1,-1))
        self.u2e_t_new = nn.Parameter(u2e_new[1].view(self.num_users+1,-1))
        # rec
        if (flag == 1) or (flag == 0):
        ...
        if (flag == 2) or (flag == 0):
            ...
            # multi head
            mul_seq = torch.cat([att(self.u2e_t_new, inputs, seq_l) for att in self.in_att], dim=2).cuda()
            mul_seq_c = torch.cat([mul_seq[i] for i in range(mul_seq.size()[0])], dim=0)
            mul_one = torch.mm(mul_seq_c, self.w)
            mul_one = F.elu(mul_one)
            seq_hidden_att = self.out_att(self.u2e_t_new,mul_one.view(mul_seq.size()[0], mul_seq.size()[1], self.hidden_size),seq_l).cuda()
