...
class TRUST_PATH(nn.Module):
    def __init__(self, user_num, nb_heads, hidden_size):
        ...
        # multi head
        self.in_att = [GraphAttentionLayer(hidden_size, concat=True) for _ in range(nb_heads)]
        for i, attention in enumerate(self.in_att):
            self.add_module('attention_{}'.format(i), attention)
        self.out_att = GraphAttentionLayer(hidden_size, concat=False)
        self.w = nn.Parameter(torch.zeros(size=(nb_heads * self.hidden_size, self.hidden_size)))
        nn.init.xavier_uniform_(self.w.data, gain=1.414)

        self.trust_user_embs = nn.Embedding(user_num + 1, self.hidden_size)
    ...
    def forward(self, slice_indices, data, flag):
        if flag == 0:
