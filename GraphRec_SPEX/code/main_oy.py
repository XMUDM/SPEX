...
class Graph_Session(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.v2e = nn.Embedding(num_items + 1, self.embed_dim)
        # trust
    ...
    def forward(self, nodes_u, nodes_v, labels_list, i, data, flag=0):
        ...
        u2e_r_new_w = self.mlp_r(torch.cat([self.u2e_r.weight, share_u_r], dim=1))
        u2e_t_new_w = self.mlp_t(torch.cat([self.u2e_t.weight, share_u_t], dim=1))
        self.u2e_r_new = nn.Embedding(num_users + 1, self.hidden_size, _weight=nn.Parameter(u2e_r_new_w)).to(device)
        self.u2e_t_new = nn.Embedding(num_users + 1, self.hidden_size, _weight=nn.Parameter(u2e_t_new_w)).to(device)
        ...
        if (flag == 2) or (flag == 0):
            ...
            seq_l = torch.sum(mask, 1)  # trust路径长度
            # multi head
...
def Test(model, epoch, device, test_loader, test_data2, best_rec, best_epoch, best_result):
    ...
    perf_rec = 'Rec:  Epoch %d : recall=[%.4f, %.4f, %.4f],  ndcg=[%.4f, %.4f, %.4f]' % (
        epoch, result_rec['recall'][0], result_rec['recall'][1], result_rec['recall'][2], result_rec['ndcg'][0], result_rec['ndcg'][1],
        result_rec['ndcg'][2])
