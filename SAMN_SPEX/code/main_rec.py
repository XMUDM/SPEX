...
from utility1.batch_test import test_rec, test_trust
from utility1.Model import SAMN_TRUST
...
def train(model, optimizer, train_loader, epoch):
    ...
    for data in train_loader:
        ...
        loss = model(batch_user.cuda(), batch_item.cuda(), label.cuda(), batch_uf.cuda(), None, None, 0)
        total_loss += loss.item()
        loss.backward()
        optimizer.step()
    log.record('%d, %.5f' % (epoch, total_loss))
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        log.record('recall=[%.5f, %.5f, %.5f],  ndcg=[%.5f, %.5f, %.5f]' % (result['recall'][0], result['recall'][1], result['recall'][2], result['ndcg'][0], result['ndcg'][1],result['ndcg'][2]))


    log.record("--- Train Best ---")
    best_rec = 'recall=[%.5f, %.5f, %.5f],  ndcg=[%.5f, %.5f, %.5f]' % (
    rec_bres[0], rec_bres[1], rec_bres[2], rec_bres[3], rec_bres[4],rec_bres[5])
    log.record("Rec : ", best_rec)
