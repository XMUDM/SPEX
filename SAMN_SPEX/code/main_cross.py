...
class SAMN_TRUST(nn.Module):
    ...
    def forward(self, input_u, input_i, label, input_uf, i, data, flag):
        u2e_new = torch.matmul(self.share_p, torch.cat([self.u2e_r.view(1,-1), self.u2e_t.view(1,-1)], dim=0))
        self.uidW_r = u2e_new[0].view(self.user_num+1,-1)
        self.uidW_t = u2e_new[1].view(self.user_num+1,-1)
        ...
        if (flag == 2) or (flag == 0):
            ...
            seq_hidden_att = self.out_att(self.uidW_t,mul_one.view(mul_seq.size()[0],mul_seq.size()[1], self.embedding_size),seq_l).cuda()
