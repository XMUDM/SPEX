...
from utility.model_tps import NCF
...
for epoch in range(args.epochs):
    ...
    for data in train_loader:
        ...
        #path_index = set(path_index) & avail_path_index

        if len(path_index) > trust_batch_size:
            path_index = random.sample(path_index, trust_batch_size)
            #avail_path_index = avail_path_index - set(path_index)

        # Hui: No need to sample from avail_path_index, although not all paths will be trained once.
        # We do so to make sure that all the trust paths start with users in this batch.
        # else:
        #     no_u_path_index = random.sample(avail_path_index - path_index, trust_batch_size - len(path_index))
        #     path_index_sample = path_index | set(no_u_path_index)
        #     avail_path_index = avail_path_index - set(path_index_sample)
