...
class MultiTaskModel(tf.keras.Model):
    def __init__(self, conf, auto=True):
        ...
        self.user_embedding = tf.Variable(tf.random.normal([self.conf.num_users+1, self.conf.hidden_size], stddev=0.01),
                                          name='user_embedding')
    ...
    def call(self, user_input, item_input, labels_input, dataset, slice_indices, flag):
        if (flag == 0) or (flag == 1):
            ...
            self.final_user_embedding = tf.concat(
                [first_gcn_user_embedding, second_gcn_user_embedding, self.user_embedding[:-1]], 1)
        ...
        if (flag == 0) or (flag == 2):
            ...
            a = tf.reduce_sum(
                alpha * seq_hidden * tf.reshape(tf.cast(mask, dtype=tf.float32), [mask.get_shape()[0], -1, 1]),
                1)  # float
            ...
            if (flag == 2):
                return scores, negs
            targets = tf.keras.utils.to_categorical(targets, num_classes=self.conf.num_users)
            loss2 = tf.losses.categorical_crossentropy(y_true=targets, y_pred=tf.nn.softmax(scores, axis=1))
