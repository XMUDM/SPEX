...
class MultiTaskModel(tf.keras.Model):
    def __init__(self, conf, auto=True):
        ...
        # oy --------------------------------------------------------------------------------
        self.u2e_r = tf.Variable(tf.random.normal([self.conf.num_users + 1, self.conf.hidden_size], stddev=0.01))
        self.u2e_t = tf.Variable(tf.random.normal([self.conf.num_users + 1, self.conf.hidden_size], stddev=0.01))
        # share
        self.mlp = Dense(units=self.conf.hidden_size, use_bias=True, activation=tf.nn.relu)
        self.mlp_r = Dense(units=self.conf.hidden_size, use_bias=True, activation=tf.nn.relu)
        self.mlp_t = Dense(units=self.conf.hidden_size, use_bias=True, activation=tf.nn.relu)
        # ------------------------------------------------------------------------------------
    ...
    def call(self, user_input, item_input, labels_input, dataset, slice_indices, flag):
        ...
        self.uidW_r = self.mlp_r(tf.concat([self.u2e_r, share_u_r], 1))
        self.uidW_t = self.mlp_t(tf.concat([self.u2e_t, share_u_t], 1))
        ...
        if (flag == 0) or (flag == 1):
            ...
            self.fusion_user_embedding = self.uidW_r[:-1]
            ...
            self.final_user_embedding = tf.concat(
                [first_gcn_user_embedding, second_gcn_user_embedding, self.uidW_r[:-1]], 1)
        ...
        if (flag == 0) or (flag == 2):
            ...
            mul_seq = tf.concat([att(self.uidW_t, path, seq_l) for att in self.in_att], 2)
            ...
            seq_hidden = self.out_att(self.uidW_t, tf.reshape(mul_one,
            ...
            a = tf.reduce_sum(
                alpha * seq_hidden * tf.reshape(tf.cast(mask, dtype=tf.float32), [mask.get_shape()[0], -1, 1]),1)
            if not self.conf.nonhybrid:
                a = self.linear_transform(tf.concat([a, ht], 1))
            b = self.uidW_t[:-1]  # n_nodes x latent_size
