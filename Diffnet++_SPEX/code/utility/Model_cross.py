...
class MultiTaskModel(tf.keras.Model):
    def __init__(self, conf, auto=True):
        ...
        self.share_p = tf.Variable(tf.random.normal([2, 2]))
        # -----------------------------------------
    ...
    def call(self, user_input, item_input, labels_input, dataset, slice_indices, flag):
        u2e_new = tf.matmul(self.share_p, tf.concat([tf.reshape(self.u2e_r,[1,-1]), tf.reshape(self.u2e_t,[1,-1])], 0))
        self.uidW_r = tf.reshape(u2e_new[0],[self.conf.num_users + 1, -1])
        self.uidW_t = tf.reshape(u2e_new[1],[self.conf.num_users + 1, -1])
