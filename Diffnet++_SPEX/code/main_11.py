...
from utility.Model_multi import MultiTaskModel
...
def start(conf, d_train, d_test, train_data, test_data, user_path_indx, alluser_path_index, model, optimizer):
    ...
    # ----------------------
    # define log name
    log_path = os.path.join(os.getcwd(), 'log/%s_11.log' % (conf.dataset))
    ...
    for epoch in range(1, conf.epochs + 1):
        ...
        t1 = time()
        log.record('%d,%.5f,%.5f,Time[%.4f]' % (epoch, total_loss1, total_loss2, t1 - t0))
        ...
        t3 = time()
        log.record('epoch[%d], Recall[%.5f,%.5f,%.5f], NDCG:[%.5f,%.5f,%.5f], Time:[%.4f]' % (
...
if __name__ == "__main__":
    ...
    model = MultiTaskModel(args,False)
