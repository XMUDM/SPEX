...
class DANSER(nn.Module):
    ...
    def __init__(self, user_count, item_count, args, train=True):
        ...
        self.task_weights = nn.Parameter(torch.tensor([0.5, 0.5]))

        # trust
        self.nonhybrid = args.nonhybrid
        self.linear_one = nn.Linear(self.hidden_units_u, self.hidden_units_u, bias=True)
        self.linear_two = nn.Linear(self.hidden_units_u, self.hidden_units_u, bias=True)
        self.linear_three = nn.Linear(self.hidden_units_u, 1, bias=False)
        self.linear_transform = nn.Linear(self.hidden_units_u * 2, self.hidden_units_u, bias=True)
        self.reset_parameters()  
        self.in_att = [GraphAttentionLayer(self.hidden_units_u, concat=True) for _ in range(args.nb_heads)]
        for i, attention in enumerate(self.in_att):
            self.add_module('attention_{}'.format(i), attention)
        self.out_att = GraphAttentionLayer(self.hidden_units_u, concat=False)
        self.w = nn.Parameter(torch.zeros(size=(args.nb_heads * self.hidden_units_u, self.hidden_units_u)))
        nn.init.xavier_uniform_(self.w.data, gain=1.414)
        self.criterion2 = nn.CrossEntropyLoss()  

        # rec
        self.learning_rate = nn.Parameter(torch.FloatTensor())
        self.training = train
        ...
        # expert_s------------------------------------
        self.att_t = nn.Parameter(torch.zeros(size=(2 * self.hidden_units_u, 2)))
        nn.init.xavier_uniform_(self.att_t.data, gain=1)
        # -----------------------------------------

    def reset_parameters(self):
        stdv = 1.0 / math.sqrt(self.hidden_units_u)
        for weight in self.parameters():
            weight.data.uniform_(-stdv, stdv)

    def reg_loss(self):
        return torch.norm(input=self.user_emb_w.weight, p=2)+torch.norm(input=self.item_emb_w.weight, p=2)

    def forward(self, u, i, l, u_read, u_friend, uf_read,u_read_l,u_friend_l,uf_read_l, i_read, i_friend, if_read, i_link, i_read_l, i_friend_l, if_read_l,index, data, flag):
        ...
        if (flag == 1) or (flag == 0) or (flag == 3):
            ...
            key_masks = torch.tensor(
                [[True] * i + [False] * (fuid_emb.shape[1] - i) for i in u_friend_l]).cuda()  # [B, F]
            ...
            key_masks = torch.tensor(
                [[True] * i + [False] * (fiid_emb.shape[1] - i) for i in i_friend_l]).cuda()  # [B, F]
            key_masks = key_masks.unsqueeze(2)  # [B, F, 1]
            ...
            uid_emb_exp3 = uid_emb_exp3.repeat(1, ifr_emb.shape[1], ifr_emb.shape[2],
                                               1)  # [B, F, R, H]  ([384, 0, 991, 10])
            ...
            if finf_in.shape[1] == 0:
                ...
                finf_in = torch.max(finf_in, 2)[
                    0]  # [B, F, H]  RuntimeError: cannot perform reduction function max on tensor with no elements because the operation does not have an identity
            ...
            loss1 = self.criterion(self.score, label.float())
            if flag == 3: return loss1

        if (flag == 2) or (flag == 0):
            if flag == 0:
                inputs, mask, targets = data.get_slice(index)
            else:
                inputs, mask, targets, negs = data.get_slice(index)
            ...
            # multi head
            mul_seq = torch.cat([att(self.user_emb_w.weight, inputs, seq_l) for att in self.in_att], dim=2).cuda()
            ...
            seq_hidden_att = self.out_att(self.user_emb_w.weight,
                                          mul_one.view(mul_seq.size()[0], mul_seq.size()[1], self.hidden_units_u),
                                          seq_l).cuda()
    ...
    def compute_scores(self, hidden, inputs, mask):
        ht = hidden[torch.arange(mask.shape[0]).long(), torch.sum(mask, 1) - 1]  # train_batch_size x latent_size
        q1 = self.linear_one(ht).view(ht.shape[0], 1, ht.shape[1])  # train_batch_size x 1 x latent_size
        q2 = self.linear_two(hidden)  # train_batch_size x seq_length x latent_size
        alpha = self.linear_three(torch.sigmoid(q1 + q2))
        p_a = torch.sum(alpha * hidden * mask.view(mask.shape[0], -1, 1).float(), 1)
        if not self.nonhybrid:
            p_a = self.linear_transform(torch.cat([p_a, ht], 1))
        b = self.user_emb_w.weight[:-1]  # n_nodes x latent_size

        # ------------------------------
        p_i_all = self.user_emb_w.weight[inputs]
