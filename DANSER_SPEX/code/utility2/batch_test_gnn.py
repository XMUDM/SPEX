...
def test_trust5(model, test_data):
    ...
    with torch.no_grad():
        for slice_indices in slices:
            trust_scores, trust_targets = model(None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, slice_indices, test_data, 2)
...
def trust_test2(model, test_data):
    l = test_data.length
    recall10, recall20, recall50, ndcg10, ndcg20, ndcg50 = 0, 0, 0, 0, 0, 0
    slices = test_data.generate_batch(256)
...
def trust_test(model, test_data):
    ...
    with torch.no_grad():
        for slice_indices in slices:
            trust_scores, trust_targets = model(None, None, None, None, None, None, None, None, None, slice_indices, test_data, 2)
            sub_indexs = trust_scores.topk(50)[1]
            sub_indexs = sub_indexs.cpu().detach().numpy()
            for index, target in zip(sub_indexs, trust_targets.cpu().detach().numpy()):
                re = test_one_user([target], index)
...
def trust_test3(model, test_data):
    l = test_data.length
    recall10, recall20, recall50, ndcg10, ndcg20, ndcg50 = 0, 0, 0, 0, 0, 0
    slices = test_data.generate_batch(256)
    ...
    with torch.no_grad():
        for slice_indices in slices:
            trust_scores, trust_targets = model(None, None, slice_indices, test_data, 2)
            sub_indexs = trust_scores.topk(50)[1]
            sub_indexs = sub_indexs.cpu().detach().numpy()
            for index, target in zip(sub_indexs, trust_targets.cpu().detach().numpy()):
                re = test_one_user([target], index)
