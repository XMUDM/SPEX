...
class Model_Wrapper(nn.Module):
    ...
    def forward(self, user, item, labels_list, trust_data, slice_indices, flag):
        ...
        self.u2e_r_new = nn.Parameter(u2e_new[0].view(self.n_users+1,-1)[:self.n_users])
        self.u2e_t_new = nn.Parameter(u2e_new[1].view(self.n_users+1,-1))
...
def train(model, optimizer):
    ...
    for epoch in range(args.epoch):
        ...
        t2 = time()
        train_time = t2 - t1
        print('%d,%.5f,%.5f,%.5f,%.5f' % (epoch, precision1, precision2, total_loss1, total_loss2)) # 画图文件
        model.eval()
        # rec test
...
if __name__ == '__main__':
    ...
    # print("len(user_path_indx):", len(user_path_indx))
